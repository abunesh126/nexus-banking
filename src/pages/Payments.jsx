...
function ConfirmModal({ upiId, amount, note, onConfirm, onCancel, loading }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative z-10 w-full max-w-sm bg-bg-card border border-border-card rounded-2xl p-6 shadow-xl"
           style={{ animation: "fadeIn .15s ease" }}>
        <button onClick={onCancel} className="absolute top-4 right-4 text-text-muted hover:text-text-main transition-colors">
          <X size={17} />
        </button>
        ...
        <div className="flex gap-3">
          <button onClick={onCancel}
            className="flex-1 border border-border-card text-text-muted hover:bg-bg-page rounded-xl py-2.5 text-sm font-medium transition-all duration-150">
            Cancel
          </button>
          <button onClick={onConfirm} disabled={loading}
            className="flex-1 flex items-center justify-center gap-2 bg-success hover:bg-green-700 disabled:opacity-60 text-white rounded-xl py-2.5 text-sm font-semibold transition-all duration-150">
...
function Toast({ type, message, onClose }) {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-lg border max-w-sm
      ${isSuccess ? "bg-bg-card border-success/30" : "bg-bg-card border-danger/30"}
      animate-[slideUp_.2s_ease]`}>
      {isSuccess ? <CheckCircle2 size={17} className="text-success" /> : <XCircle size={17} className="text-danger" />}
      <p className="text-sm font-medium text-text-main">{message}</p>
      <button onClick={onClose} className="ml-2 text-text-muted hover:text-text-main"><X size={13} /></button>
    </div>
  );
}
    ...
    <>
      ...
      <div className="space-y-5 max-w-4xl mx-auto page-enter">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-text-main">Payments & UPI</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          ...
          {/* Form */}
          <div className="lg:col-span-2 bg-bg-card border border-border-card rounded-2xl p-6 shadow-sm h-fit">
            ...
            <div className="mb-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {QUICK_CONTACTS.map((c) => (
                  <button key={c.upiId} onClick={() => { setUpiId(c.upiId); setErrors((e) => ({ ...e, upiId: undefined })); }}
              ...
              <Field label="UPI ID" error={errors.upiId}>
                <div className="relative">
                  ...
                  <input id="upi-id" type="text" value={upiId}
                    ...
                    className={`${inputCls(errors.upiId)} pl-8`} placeholder="name@okicici" autoComplete="off" />
                </div>
              </Field>
              <Field label="Amount (₹)" error={errors.amount}>
                <div className="relative">
                  <IndianRupee size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-text-muted" />
                  <input id="upi-amount" type="number" min="1" value={amount}
                    onChange={(e) => { setAmount(e.target.value); setErrors((er) => ({ ...er, amount: undefined })); }}
                    className={`${inputCls(errors.amount)} pl-9`} placeholder="0.00" />
                </div>
              </Field>
              <Field label="Note (optional)">
                <div className="relative">
                  <FileText size={13} className="absolute left-3.5 top-3 text-text-muted" />
                  <input id="upi-note" type="text" value={note} onChange={(e) => setNote(e.target.value)}
                    className={`${inputCls(false)} pl-9`} placeholder="Rent, lunch, etc." maxLength={50} />
                </div>
              </Field>
              <button id="upi-pay-btn" type="submit"
                className="w-full flex items-center justify-center gap-2 bg-success hover:bg-green-700 text-white font-semibold rounded-xl py-2.5 mt-1 transition-all duration-150 shadow-sm">
          ...
          {/* Recent payments */}
          <div className="lg:col-span-3 bg-bg-card border border-border-card rounded-2xl p-6 shadow-sm">
            <h2 className="text-sm font-semibold text-text-main mb-4 flex items-center gap-2">
